    ...
    return (
        <div key={item.id} className="flex flex-col w-[30%] rounded-lg bg-slate-950 pb-4">
            <div className="h-[400px] flex flex-col justify-center items-center">
                <Image src={`${item.images.edges[0].node.url}`} objectFit="contain" width={300} height={300} alt={item.images.edges[0].node.altText ? item.images.edges[0].node.altText : 'image'} priority={true} />
                
            </div>
            <div className="text-meduim px-2">
                {item.title}
            </div>
